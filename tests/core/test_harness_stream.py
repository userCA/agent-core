...
def _tool_session(session_id: str = "stream") -> tuple[AgentSession, FakeProvider]:
...
async def test_set_stream_options_affects_next_turn_only():
    ...
    session, provider = _tool_session("opts-next-turn")
    ...
    async def on_event(evt):
        ...
        if evt.type == "turn_start" and not switched:
            switched = True
            session.set_stream_options({"headers": {"X-Turn": "2"}})

    session.subscribe(on_event)
    await session.start()
    session.set_stream_options({"headers": {"X-Turn": "1"}})
...
async def test_before_provider_request_hook_patches_options():
    session, provider = _tool_session("req-hook")
    provider.queue_script([
        StreamTextDelta(text="ok"),
        StreamMessageEnd(stop_reason="stop", input_tokens=1, output_tokens=1),
    ])
    await session.start()
    session.set_stream_options({"headers": {"base": "yes"}})
...
async def test_before_provider_request_hook_deletes_header():
    session, provider = _tool_session("req-delete")
    provider.queue_script([
        StreamTextDelta(text="ok"),
        StreamMessageEnd(stop_reason="stop", input_tokens=1, output_tokens=1),
    ])
    await session.start()
    session.set_stream_options({"headers": {"drop-me": "x", "keep": "y"}})
...
async def test_before_provider_payload_hook_transforms_messages():
    session, provider = _tool_session("payload-hook")
    provider.queue_script([
        StreamTextDelta(text="ok"),
        StreamMessageEnd(stop_reason="stop", input_tokens=1, output_tokens=1),
    ])
    ...
    session.hooks.on("before_provider_payload", patch_payload)
...
async def test_after_provider_response_hook_observed():
    session, provider = _tool_session("resp-hook")
    provider.queue_script([
        StreamTextDelta(text="ok"),
        StreamMessageEnd(stop_reason="stop", input_tokens=1, output_tokens=1),
    ])
