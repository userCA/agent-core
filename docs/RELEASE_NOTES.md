...
- **临时存档**：技能进化生成的改进提案现在会先临时存档，等待审批后再应用
- **阻塞机制**：存在未审批内容时，不会执行新的进化周期，避免混乱
- **用户提示**：前端会显示提示，告知用户需要先处理现有提案后才能执行新的分析
- **强制绕过**：支持强制绕过阻塞，适用于紧急情况
...
- **Markdown 渲染**：助手回复现在支持富文本渲染，代码块、标题、列表、链接等均可正确显示
- **语音输入**：输入栏新增麦克风按钮，长按录音后可发送语音消息；人工输入表单也支持录音字段
- **暗色主题生效**：设置页切换暗色主题后，所有页面均会跟随切换
- **伴侣心情**：伴侣页头像新增心情角标，可查看当前情绪状态和描述
- **长会话流畅度**：消息列表采用分页加载，滚动到顶部自动加载更早的消息

### 稳定性修复

- 修复图片上传失败时可能导致的消息内容错乱问题
- 修复录音管理器在多次进出页面后回调重复触发的问题
- 修复 Markdown 链接中危险协议（javascript:）可能被注入的安全问题
...
- **登录**：支持 UID 登录，自动记忆最近使用的账号，一键快速登录
- **对话**：发送文字消息后，实时流式显示回复内容，与 H5 端体验一致
- **推理过程展示**：可查看思考过程、工具调用、技能执行等推理步骤
- **中止功能**：生成过程中可随时点击中止按钮停止回复
- **会话历史**：查看历史会话列表，支持搜索和按日期分组，可切换和删除会话
- **人工输入**：当 Agent 需要人工确认时，弹出表单填写并提交
- **图片上传**：输入框左侧可点击选择相册图片或拍摄照片并发送
- **伴侣页面**：查看宠物的品种、稀有度、属性、亲密度、怪癖和外观信息，可互动（戳/喂/表扬）
- **技能管理**：查看和开关技能，支持创建和导入技能文件，可查看进化追踪数据和提议
- **专家切换**：设置页可选择不同专家角色，影响后续对话风格
- **主题切换**：支持亮色/暗色主题切换，自动记忆偏好
- **认证设置**：设置页可管理认证信息，支持密码字段安全输入
- **模型选择**：设置页可选择当前使用的 AI 模型
...
- **信息聚焦**：工具调用过程中生成的中间文本（如计划方案、查询结果摘要）现在自动收纳到推理卡片折叠区，不再占据主内容区域干扰用户
- **最终结果突出**：只有最终回复显示在内容卡片中，中间过程全部折叠

### 稳定性修复

- 修复多个扩展组件因方法缺失导致的运行时错误
- 修复知识库搜索工具参数不兼容问题
...
- **风格对齐**：专家协调（delegation）卡片现在与思考/工具/技能步骤完全统一，作为 trace-rail 中的一个节点渲染，消除视觉割裂感
- **可展开明细**：点击协调步骤可展开查看每位专家的名称、状态、任务和摘要，交互与其他步骤一致
- **流式输出顺序优化**：确认文本现在先于协调卡片出现，用户先看到回应再看到专家调度过程
...
- **中间过程自动折叠**：当 AI 调用工具时，中间推理步骤（思考、文本、工具结果）自动收入推理卡片，仅最终结果留在内容卡片显示
- **流式响应更流畅**：优化了流式响应结束时的过渡效果，推理卡片和内容卡片的切换不再出现闪烁或空白期

### 交互体验优化

- **确认卡片修复**：修复了确认卡片无法输入文本的问题，现在可以正常填写表单并提交
- **消除空白占位**：模型推理过程中不再出现空白消息卡片，界面更整洁
...
- **TraceCard 技能节点**：当对话中使用了 Skill（如“图片生成”），推理时间线中会显示绿色技能节点，明确标注“正在使用 xxx 技能”
- **SKILL.md tools 字段**：技能文件现在可以声明关联的工具名，系统会自动检测并触发技能激活事件
- **历史消息技能节点**：打开历史消息时，推理卡片中的技能节点现在可以正确显示，与实时对话一致

### 推理卡片视觉优化

- **推理与消息卡片视觉区分**：推理卡片采用 2px 墨色粗边框 + 浮动阴影，与普通消息气泡形成明显视觉层次
- **推理步骤摘要**：推理完成后显示有意义的描述摘要，而非简单的"完成"
- **布局间距优化**：步骤间距更宽松，推理卡片整体更清爽
